            ...
            if (videoID) {
                ...
                ytdl.getBasicInfo(videoID).then(info => {
                    if (parseInt(info.videoDetails.lengthSeconds) > 39600) {
                        msg.channel.send(':no_entry_sign: Sorry, but this fucking video is longer than 11 hours.');
                        return;
                    }
                    if (!info.videoDetails.title) {
                        msg.channel.send(`:no_entry_sign: Youtube video with ID \`${videoID}\` is not accessible. Maybe private?`);
                        return;
                    }
